...
public class GameService {
	...
	public boolean startGame(Long roomSeq) {
		...
		if (!room.isFull()) {
			log.warn("startGame: Room not full, roomSeq={}, playerCount={}", roomSeq, room.getGameUsers().size());
			return false;
		}
		...
		synchronized (room) {
			...
			log.info("startGame: Creating GameState, roomSeq={}, player1(black)={}, player2(white)={}", roomSeq,
					player1, player2);
	...
	public boolean makeMove(Long roomSeq, int row, int col, String playerNickname) {
		...
		log.info("makeMove: Attempt by {}, position=({},{}), currentTurn={}", playerNickname, row, col,
		...
		if (result) {
			...
			log.warn("makeMove: Failed! player={}, currentTurn={}", playerNickname, gameState.getCurrentTurn());
		}
	...
	public boolean isGameOver(Long roomSeq) {
		...
		if (gameOver) {
			...
			String loserName = gameState.getWinner().equals(gameState.getBlackPlayer()) ? gameState.getWhitePlayer()
			...
			log.info("isGameOver: Game is over, roomSeq={}, winner={}", roomSeq, gameState.getWinner());
		}
	...
	public void updateRankOnGameEnd(GameState gameState) {
		...
		if (winnerName == null || "DRAW".equals(winnerName))
			return;
		...
		// 패자
		String loserName = winnerName.equals(gameState.getBlackPlayer()) ? gameState.getWhitePlayer()
				: gameState.getBlackPlayer();
		User loser = userService.findByNickName(loserName);
