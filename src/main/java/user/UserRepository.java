...
public class UserRepository extends JDBCRepository<User, String> {
	...
	protected User mapRow(ResultSet rs) throws SQLException {
		return new User(rs.getLong("SEQ_ID"), rs.getString("USER_ID"), rs.getString("USER_PW"), rs.getString("EMAIL"),
	...
	public int save(User user) {
		String sql = "INSERT INTO USERS (USER_ID, USER_PW, EMAIL, NICKNAME, CREATED_AT) " + "VALUES (?, ?, ?, ?, ?)";
		// insert 문 호출
	...
	public User findById(Long seqId) {
		String sql = "SELECT " + "SEQ_ID, USER_ID, USER_PW, EMAIL, NICKNAME, CREATED_AT, DELETED_AT " + "FROM USERS "
				+ "WHERE SEQ_ID = ?";
	...
	/*
	 * rank테이블 seq_id 자동 삽입용 public Long findBySeq_Id(String id) { String sql =
	 * "SELECT " +
	 * "SEQ_ID, USER_ID, USER_PW, EMAIL, NICKNAME, CREATED_AT, DELETED_AT " +
	 * "FROM USERS " + "WHERE USER_ID = ?";
	 * 
	 * return executeQuery(sql, pstmt -> pstmt.setString(1, id), rs -> { try { if
	 * (rs.next()) { return rs.getLong("SEQ_ID"); } } catch (SQLException e) {
	...
	public int withDraw(Long id, String userPw) {
		...
		String sql = "UPDATE USERS SET DELETED_AT = NOW() " + "WHERE USER_PW = ?" + "AND SEQ_ID = ?"; // soft Delete
